...
async def list_all_users(
    ...
    current_user: ClerkUserContext = Depends(
        require_admin_permission(AdminPermission.VIEW_ALL_USERS)
    ),
...
async def create_tutor(
    ...
    current_user: ClerkUserContext = Depends(
        require_admin_permission(AdminPermission.CREATE_TUTORS)
    ),
    database: AsyncIOMotorDatabase = Depends(get_database),
):
    ...
    try:
        ...
        existing = await database.tutors.find_one(
            {"$or": [{"clerk_id": request.clerk_id}, {"email": request.email}]}
        )

        if existing:
            raise HTTPException(
                status_code=400,
                detail="User with this email or clerk_id already exists",
            )
...
async def update_user(
    ...
    current_user: ClerkUserContext = Depends(
        require_admin_permission(AdminPermission.MANAGE_USERS)
    ),
    database: AsyncIOMotorDatabase = Depends(get_database),
):
    ...
    try:
        ...
        for collection_name in ["tutors", "students", "parents"]:
            ...
            result = await collection.update_one(
                {"clerk_id": user_id}, {"$set": update_data}
            )
        ...
        for user_id in request.user_ids:
            try:
                ...
                if not user_found:
                    results.append(
                        BatchOperationResult(
                            id=user_id, success=False, error="User not found"
                        )
                    )
                    failed += 1
                    continue
                ...
                if (
                    user_doc.get("is_super_admin", False)
                    and request.operation == BatchOperationType.DELETE
                ):
                    results.append(
                        BatchOperationResult(
                            id=user_id, success=False, error="Cannot delete super admin"
                        )
                    )
                    failed += 1
                    continue
                ...
                if request.operation == BatchOperationType.ACTIVATE:
                    ...
                    results.append(
                        BatchOperationResult(
                            id=user_id,
                            success=False,
                            error="Invalid operation for users",
                        )
                    )
                    failed += 1
                    continue
                ...
                results.append(
                    BatchOperationResult(id=user_id, success=False, error=str(e))
                )
                failed += 1
